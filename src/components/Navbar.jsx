    ...
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-ink/85 backdrop-blur-md">
      <div className="section-shell flex h-20 items-center justify-between">
        ...
        <div className="hidden items-center gap-7 md:flex">
          {items.map(([name, href], i) => <a key={name} href={href} className="text-xs font-semibold uppercase tracking-[.13em] text-slate-400 transition-colors hover:text-white">0{i + 1} {name}</a>)}
          <ThemeToggle />
        </div>
