...
const groups = [
  { title: 'Full-stack', icon: Layers, image: '/skills/backend.svg', description: 'End-to-end web applications, APIs and application architecture', items: ['React', 'Node.js', 'NestJS', 'TypeScript', 'REST APIs', 'API Integration', 'Authentication', 'Responsive UI'] },
  { title: 'AI / ML', icon: Brain, image: '/skills/ai-ml.svg', description: 'Machine learning and practical AI-powered applications', items: ['Python', 'Machine Learning', 'LLMs', 'NLP', 'GenAI', 'LangChain', 'Model Evaluation', 'Streamlit'] },
  { title: 'Engineering', icon: Wrench, image: '/skills/engineering.svg', description: 'Reliable software development, data and delivery practices', items: ['Clean Code', 'Problem Solving', 'System Design', 'SQL', 'MySQL', 'Git & GitHub', 'Docker', 'Linux'] },
  { title: 'Frontend', icon: Code2, image: '/skills/frontend.svg', description: 'Responsive interfaces and polished digital experiences', items: ['JavaScript', 'HTML5', 'CSS3', 'React', 'Figma', 'Flexbox', 'CSS Grid', 'Accessibility'] },
];
          ...
          {groups.map(({ title, icon: Icon, image, description, items }, index) => (
            <motion.article key={title} initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ delay: index * .1, duration: .55 }} whileHover={{ y: -6 }} className="group overflow-hidden border border-black/10 bg-paper dark:border-white/10 dark:bg-night">
              <div className="relative h-44 overflow-hidden sm:h-52">
                <motion.img src={image} alt="" aria-hidden="true" whileHover={{ scale: 1.05 }} transition={{ duration: .6 }} className="h-full w-full object-cover" />
