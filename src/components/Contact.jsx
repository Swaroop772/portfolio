...
const Contact = () => {
  ...
  return (
    <section id="contact" className="relative overflow-hidden border-t border-black/10 py-28 dark:border-white/10 sm:py-36">
      <div className="section-shell">
        <div className="grid gap-14 lg:grid-cols-[.9fr_1.1fr] lg:gap-24">
          <motion.div initial={{ opacity: 0, x: -35 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .3 }} transition={{ duration: .7 }}>
            ...
            <p className="mt-7 max-w-md text-base leading-7 text-black/55 dark:text-white/55">Have an idea, opportunity or just want to talk tech? Drop me a message and I'll get back to you.</p>
            <motion.a whileHover={{ x: 6 }} href="mailto:swaroopkurapati15@gmail.com" className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-teal">swaroopkurapati15@gmail.com <ArrowUpRight size={15} /></motion.a>
          </motion.div>
          <motion.form ref={formRef} onSubmit={handleSubmit} initial={{ opacity: 0, x: 35 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .25 }} transition={{ duration: .7, delay: .1 }} className="space-y-7">
            {[['name','Your name','John Doe','text'],['email','Email address','john@example.com','email']].map(([field,label,placeholder,type], i) => <motion.label key={field} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: .2 + i * .1 }} className="block"><span className="form-label">{label}</span><input name={field} type={type} value={formData[field]} onChange={update(field)} required className="form-input" placeholder={placeholder} /></motion.label>)}
            <motion.label initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: .4 }} className="block"><span className="form-label">Tell me about it</span><textarea name="message" rows="5" value={formData.message} onChange={update('message')} required className="form-input resize-none" placeholder="I have an idea for..." /></motion.label>
            ...
            <motion.button whileHover={{ y: -2 }} whileTap={{ scale: .97 }} disabled={isSending} className="btn-primary w-full sm:w-auto">{isSending ? <>Sending <Loader2 size={16} className="animate-spin" /></> : <>Send message <Send size={16} /></>}</motion.button>
          </motion.form>
        </div>
      </div>
    </section>
