    ...
    <section id="projects" className="relative overflow-hidden bg-surface py-28 sm:py-36">
      ...
      <div className="section-shell relative z-10">
        <div className="flex flex-col justify-between gap-7 md:flex-row md:items-end">
          <div><p className="section-kicker">02 — Selected work</p><h2 className="section-heading mt-4">My recent <span>works</span></h2></div>
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {projects.map((project, index) => (
            <motion.article key={project.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .08 }} onClick={() => setSelectedProject(project)} className="group cursor-pointer">
              ...
              <div className="mt-5 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-teal">{project.type}</p><h3 className="mt-2 text-xl font-bold text-white">{project.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{project.description}</p></div><span className="mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 text-slate-500 transition-all group-hover:border-coral group-hover:text-coral"><ArrowUpRight size={16} /></span></div>
