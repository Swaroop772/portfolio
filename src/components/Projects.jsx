...
const ProjectCard=({project,index,onOpen})=>{
 ...
 const rotateX=useSpring(useTransform(my,[-120,120],[2.5,-2.5]),{stiffness:250,damping:25});
 const rotateY=useSpring(useTransform(mx,[-120,120],[-2.5,2.5]),{stiffness:250,damping:25});
 ...
 return <motion.article initial={{opacity:0,y:32}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:'-80px'}} transition={{delay:index*.1,duration:.6}} onMouseMove={move} onMouseLeave={reset} onClick={()=>onOpen(project)} style={{rotateX,rotateY,transformPerspective:1000}} className="group cursor-pointer">
  <motion.div whileHover={{y:-6}} className="relative aspect-[1.12/1] overflow-hidden" style={{backgroundColor:color.bg}}>
   <div className="absolute inset-0 opacity-[.14]" style={{backgroundImage:'radial-gradient(circle,#fff 1px,transparent 1px)',backgroundSize:'20px 20px'}} />
   <motion.div whileHover={{scale:1.035,rotate:-1}} className="absolute left-[10%] top-[13%] h-[65%] w-[80%] overflow-hidden bg-[#f4f1eb] p-4 shadow-2xl">
   ...
   <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between text-white"><p className="text-2xl font-black uppercase tracking-[-.04em]">{project.title}</p><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#151515]"><ArrowUpRight size={15}/></span></div>
   <motion.a href={project.github} target="_blank" rel="noreferrer" data-cursor="view" aria-label={`Open ${project.title} on GitHub`} onClick={e=>e.stopPropagation()} whileHover={{scale:1.08,rotate:-6}} className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-full bg-[#151515] text-white opacity-0 transition-opacity group-hover:opacity-100"><Github size={14}/></motion.a>
  </motion.div>
  <div className="mt-5"><p className="text-[10px] font-bold uppercase tracking-[.18em]" style={{color:color.bg}}>{project.type}</p><h3 className="mt-2 text-2xl font-black tracking-[-.04em] dark:text-[#f5f2ec]">{project.title}</h3><p className="mt-2 max-w-sm text-sm leading-6 text-black/55 dark:text-white/50">{project.description}</p></div>
...
export default function Projects(){
 ...
 return <section id="projects" className="relative overflow-hidden bg-[#ebe7df] py-28 dark:bg-[#181818] sm:py-36">
  <div className="section-shell relative z-10">
   <motion.div initial={{opacity:0,y:25}} whileInView={{opacity:1,y:0}} viewport={{once:true}}><p className="section-kicker">Selected work</p><div className="mt-5 flex flex-col justify-between gap-8 md:flex-row md:items-end"><h2 className="section-heading">Things I've<br/><span>built.</span></h2><p className="max-w-md text-sm leading-6 text-black/50 dark:text-white/50">A small selection of projects across GenAI, machine learning and cybersecurity.</p></div></motion.div>
   <div className="mt-16 grid gap-x-10 gap-y-16 md:grid-cols-2 lg:grid-cols-3" style={{perspective:1200}}>{projects.map((project,index)=><ProjectCard key={project.title} project={project} index={index} onOpen={setSelectedProject}/>)}</div>
  </div>
