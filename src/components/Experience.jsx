      ...
      <div className="section-shell">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="mt-4 flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <h2 className="section-heading max-w-2xl">Internships that<br /><span>shaped my work.</span></h2>
            <p className="max-w-md text-sm leading-6 text-slate-500">A practical mix of backend engineering, AI/ML, cybersecurity and front-end development experience.</p>
          </div>
        </motion.div>

        <div className="mt-14 divide-y divide-white/10 border-y border-white/10">
          {internships.map((item, index) => (
            <motion.article key={`${item.company}-${item.role}-${index}`} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.06 }} className="grid gap-6 py-8 lg:grid-cols-[1.05fr_1.45fr_auto] lg:items-start">
              <div>
                <div className="flex items-center gap-3">
                  <span className="flex h-9 w-9 items-center justify-center border border-teal/30 text-teal"><Briefcase size={16} /></span>
                  <div>
                    <h3 className="text-lg font-bold text-white">{item.role}</h3>
                    <p className="mt-1 text-sm font-semibold text-teal">{item.company}</p>
                  </div>
                </div>
              </div>
              <ul className="space-y-2 text-sm leading-6 text-slate-400">
                {item.points.map((point) => <li key={point} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 bg-coral" />{point}</li>)}
              </ul>
              <div className="text-left lg:text-right">
                <p className="font-mono text-xs uppercase tracking-wider text-slate-300">{item.period}</p>
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-600 lg:justify-end">
                  {item.current && <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-teal" />}
                  {item.mode}
                </div>
              </div>
            </motion.article>
        ...
        <a href="#contact" className="mt-8 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[.16em] text-teal hover:text-white">Let's work together <ArrowUpRight size={16} /></a>
      </div>
    </section>
  );
}
